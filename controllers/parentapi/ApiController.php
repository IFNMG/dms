...
class ApiController extends \yii\web\Controller {
  ...
  public $allowGuestRequest=array(
      ...
      'user/Api/v1/user/sociallogin',
      'user/Api/v1/user/forgotpassword',
      'navitrex/Api/v1/navitrex/getgpx',
      );
  ...
  protected function setAuthorizeToken() {
   ...
   public function behaviors()
    ...
    { 
        ...
        if (isset($this->actions[$action])) {
            ...
            $this->response($status=0,$error_code=400,$message='Bad Request'); 
        }
    ...
    private function isGuestUser(){ 
        ...
        if(isset($tokenArray->userId)){
            // authenticated user                                  
            $userData=\app\models\Users::find()->select('id,status')->where(['id'=>$tokenArray->userId,'is_delete'=>'1'])->one();            
            if($userData){
              ...
              //  $DEVICE_TOKEN_OPTIONAL=Yii::$app->params['configurations']['DEVICE_TOKEN_OPTIONAL'];   
                ...
                if($valid){
                    ...
                    if($globalApiKey===$res->value){
                        ...
                        $this->response($status=0,$error_code=401,$message='Unauthorized request2');                           
                    }   
                }else{
                    $this->response($status=0,$error_code=400,$message='Bad request');                           
                }
                ...
                if(isset($headers['AuthorizedToken'])){
                    ...
                    if(empty($result)){                        
                        $this->response($status=0,$error_code=401,$message='Unauthorized request3');
                    }
                    // if login request then return true in else cond;
                    if($this->globalAuthorizedToken===$result->current_token){
                        return true;
                    } else if($this->globalAuthorizedToken===$result->previous_token){
                        return true;
                    } else {                     
                        $this->response($status=0,$error_code=401,$message='Unauthorized request4');
                    }
                }
                else{
                    $this->response($status=0,$error_code=400,$message='Bad request');
   ...
   /**
    ...
    public function response($status,$code=NULL,$message,$data=NULL){       
        //get current token of requested device
        
        
        if($status==1){  
            $api_token=self::authenticateTokenTimeStamp($code,$data); 
            $response=array('status'=>$status,'code'=>$code,'data'=>$data,'message'=>$message,'api_current_token'=>$api_token);            
        }  else {
            $data=array();       
            ...
            if(!in_array($code, $codeArr)){
                ...
                $response=array('status'=>$status,'code'=>$code,'data'=>$data,'message'=>$message);
            }
        }
          //$this->setHeader($code);
        $this->setHeader(200);
       ...
       //  if(!array_key_exists('API_CURRENT_TOKEN',$data) && $api_current_token!=""){
           if(!isset($data['API_CURRENT_TOKEN']) && $api_current_token!=""){
            ...
            if(isset($tokenArray->currentTime)){
             ...
             $diffMins=\app\models\Configurations::find()->select('value')->where(['short_code'=>'API_TOKEN_EXPIRY_MINUTES'])->one();          
